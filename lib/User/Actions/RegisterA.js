...
class RegisterA extends BaseActions_1.BaseActions {
    ...
    async registerByLoginAndPass(data) {
        ...
        try {
            if (!data.login) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'emptyLogin');
            }
            if (!data.pass) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'emptypass');
            }
            if (!data.passConfirm) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'emptypassConfirm');
            }
            if (!this.object.errorSys.isOk()) {
                throw 'error input';
            }
            if (data.login.length < 5) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'loginToShort');
            }
            if (data.pass.length < 7) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'passToShort');
            }
            if (data.passConfirm != data.pass) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'passConfirm');
            }
            if (!this.object.errorSys.isOk()) {
            ...
            if (existUser) {
                this.object.errorSys.error(this.fClassName() + '.registerByLoginAndPass', 'loginAlreadyUsed');
            }
            if (!this.object.errorSys.isOk()) {
